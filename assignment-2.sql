...
select  course_name,count(e.student_id) as noOfStudents
from enrollments e 
join courses c on c.course_id = e.course_id 
...
select avg(s.student_age) from students s 
join enrollments e on e.student_id = s.student_id 
where e.course_id = (select course_id from courses c where course_name = 'Science')
